...
@section('content')
    ...
    <h1 class="text-center">Modificar cervecería</h1>

    <x-card size="M" title="Indica la información de la cervecería">

        <x-slot:content>
            <form method="POST" action="{{ route ('brewery.update', $brewery->id) }}" enctype="multipart/form-data"> 
                @method('PUT')
                @csrf

              <div class="mb-3">
                <label for="name" class="form-label">Nombre de la cervecería</label>
                <input type="text" class="form-control" id="name" name="name" placeholder="¿Cómo se llama la cervecería?" value="{{ $brewery->name }}">
              </div>
              <div class="mb-3">
                <label for="description" class="form-label">Descripción</label>
                <textarea class="form-control" id="description" name="description" rows="3" placeholder="¿Qué la hace especial?">{{ $brewery->description }}</textarea>
              </div>
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="lat" name="lat" placeholder="¿Donde está?"  value="{{ $brewery->lat }}">
              </div>
              <div class="mb-3">
                <label for="long" class="form-label">Longitud</label>
                <input type="text" class="form-control" id="long" name="long" placeholder="¿Donde está?"  value="{{ $brewery->long }}">
              </div>
              <div class="mb-3">
